...
#if (DEBUG == 1)
#define WhoIsYourDaddy 1 // 0：开启 1：关闭
#else
#define WhoIsYourDaddy 0
#endif
...
- (void)hookForwardInvocation:(NSInvocation*)invocation
{
    ...
    NSSafeProxy *safeP = [NSSafeProxy new];
    [safeP dealException:info];
    [safeP release];
}
...
+ (void)load
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        [obj swizzleInstanceMethod:@selector(setHidden:) withMethod:@selector(memeHookSetHidden:)];
        [obj release];
    });
}
- (void)memeHookSetHidden:(BOOL)value
{
    if (NSThread.currentThread != NSThread.mainThread) {
        NSLog(@"error sethidden not in main thread");
        NSString* log = [self getViewStacksLog];
        
        if(self.superview != nil && [self.superview isMemberOfClass:[UIButton class]]) {
            UIButton* btn = (UIButton*)self.superview;
        }
    }
    [self memeHookSetHidden:value];
}
