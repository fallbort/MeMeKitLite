...
- (instancetype)initWithGestureView:(UIView *)gestureView bgView:(UIView *)bgView {
    if (self = [super init]) {
        ...
        self.gestureView.transform = CGAffineTransformMakeScale(1,1);
        self.bgView = bgView;
...
- (void)panAction:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        CGFloat distanceX = insidePoint.x - self.gestureView.frame.size.width * 0.5;
        CGFloat distanceY = insidePoint.y - self.gestureView.frame.size.height * 0.5;
        ...
        //背景的一半高度作为参照
        CGFloat halfHeight = self.bgView.frame.size.height * 0.5;
        CGFloat halfWidth = self.bgView.frame.size.width * 0.5;
        ...
        //transform的scale
        CGFloat frameScale = MIN((1 - 0.5 * scale), 1);
