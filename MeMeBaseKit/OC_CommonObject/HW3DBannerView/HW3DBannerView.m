...
- (void)setInfoByCurrentImageIndex:(NSUInteger)currentImageIndex {
    ...
    if([self isHttpString:self.data[currentImageIndex]]){
        ...
        [self.centerIV yy_setImageWithURL:[NSURL URLWithString:self.data[currentImageIndex]] placeholder:image];
    }else {
        if ([self.data[currentImageIndex] isKindOfClass:[UIImage class]]) {
            self.centerIV.image = self.data[currentImageIndex];
        }else{
            self.centerIV.image = self.placeHolderImage;
        }
    }
    if (self.layoutCardViewBlock) {
        self.layoutCardViewBlock(self.centerIV,currentImageIndex);
    }
    ...
    if (self.imgCount > 1) {
        ...
        if([self isHttpString:self.data[leftIndex]]){
            UIImage* image = self.placeHolderImage;
            [self.leftIV yy_setImageWithURL:[NSURL URLWithString:self.data[leftIndex]] placeholder:image];
        }else {
            if ([self.data[leftIndex] isKindOfClass:[UIImage class]]) {
                self.leftIV.image = self.data[leftIndex];
            }else{
                self.leftIV.image = self.placeHolderImage;
            }
        }
        if (self.layoutCardViewBlock) {
            self.layoutCardViewBlock(self.leftIV,leftIndex);
        }
        
        NSInteger rightIndex = (unsigned long)((_currentImageIndex + 1) % self.imgCount);
        if([self isHttpString:self.data[rightIndex]]){
            UIImage* image = self.placeHolderImage;
            [self.rightIV yy_setImageWithURL:[NSURL URLWithString:self.data[rightIndex]] placeholder:image];
        }else {
            if ([self.data[rightIndex] isKindOfClass:[UIImage class]]) {
                self.rightIV.image = self.data[rightIndex];
            }else{
                self.rightIV.image = self.placeHolderImage;
            }
        }
        if (self.layoutCardViewBlock) {
            self.layoutCardViewBlock(self.rightIV,rightIndex);
        }
    }else{
...
- (void)reloadImage {
    ...
    if (contentOffset.x > ZXMainScrollViewWidth) { //向左滑动
        self.currentImageIndex = (_currentImageIndex + 1) % self.imgCount;
    } else if (contentOffset.x < ZXMainScrollViewWidth) { //向右滑动
        self.currentImageIndex = (_currentImageIndex - 1 + self.imgCount) % self.imgCount;
    }
...
-(void)setCurrentImageIndex:(NSUInteger)currentImageIndex {
    NSInteger oldIndex = _currentImageIndex;
    _currentImageIndex = currentImageIndex;
    if (self.indexChangedBlock) {
        self.indexChangedBlock(currentImageIndex,oldIndex);
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (self.scrollImageBlock) {
        self.scrollImageBlock(self.currentImageIndex);
    }
}
...
-(void)setItemMargnPadding:(CGFloat)itemMargnPadding {
    ...
    self.mainScrollView.frame = CGRectMake((self.bounds.size.width - (self.imgWidth + itemMargnPadding))/2, 0, self.imgWidth + itemMargnPadding, ZXMainScrollViewHeight);
    [self updateViewFrameSetting];
}

-(void)layoutSubviews {
    [super layoutSubviews];
    self.mainScrollView.frame = CGRectMake((self.bounds.size.width - (self.imgWidth + self.itemMargnPadding))/2, 0, self.imgWidth + self.itemMargnPadding, ZXMainScrollViewHeight);
    [self updateViewFrameSetting];
}
