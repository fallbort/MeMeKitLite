...
+ (MMGIFManager *)shared;
- (void)stopGIFView:(MMGIFImageView *)view;
@end
@implementation MMGIFManager
+ (MMGIFManager *)shared{
    static MMGIFManager *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[MMGIFManager alloc] init];
    });
...
- (void)play{
    for (MMGIFImageView *imageView in _gifViewHashTable) {
...
- (void)startGIF
{
    if ([[MMGIFManager shared].gifViewHashTable containsObject:self]) {return; }
...
- (void)startGIFWithRunLoopMode:(NSString * const)runLoopMode andImageDidLoad:(void(^)(CGSize imageSize))didLoad
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        if (![[MMGIFManager shared].gifViewHashTable containsObject:self]) {
            if ((self.gifData || self.gifPath)) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [[MMGIFManager shared].gifViewHashTable addObject:self];
                    _gifSourceRef = gifSourceRef;
                    _frameCount = CGImageSourceGetCount(gifSourceRef);
                    if (![MMGIFManager shared].displayLink) {
                        [MMGIFManager shared].displayLink = [CADisplayLink displayLinkWithTarget:[MMGIFManager shared] selector:@selector(play)];
                        [[MMGIFManager shared].displayLink addToRunLoop:[NSRunLoop mainRunLoop] forMode:runLoopMode];
                    }
                });
...
- (void)stopGIF{
    ...
    [self.renderQueue addOperationWithBlock:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [[MMGIFManager shared] stopGIFView:self];
        });
    }];
}
...
- (void)play{
    ...
    _timestamp += [MMGIFManager shared].displayLink.duration;
}
